...
meansFlexConC1S <- c()
method <- "Co-Training Dwcs - Selecting"
...
for (dataset in databases) {
  ...
  for (fold in folds) {
    ...
    co_training <- coTrainingDwscSelection(myModel, myFuncs, data1, data2)
  ...
  writeArchive("coTrainingMediaDwcs_selecting.txt", "../", dataName, method, acc_co,
               fscore_co, preci_co, recall_co, begin, end)
  writeArchive("coTrainingVisao1Dwcs_selecting.txt", "../", dataName, method,
               acc_co_v1, fscore_co_v1, preci_co_v1, recall_co_v1, begin, end)
  writeArchive("coTrainingVisao2wcs_selecting.txt", "../", dataName, method,
