...
meansFlexConC1S <- c()
method <- "Co-Training EbAL V2 DwS - A - Select"
...
for (dataset in databases) {
  ...
  for (fold in folds) {
    ...
    co_training <- coTrainingEbalV2DwcaSelect(myModel, myFuncs, data1, data2)
  ...
  writeArchive("coTrainingMediaEbALV2Dwsa_select.txt", "../", dataName, method, acc_co,
               fscore_co, preci_co, recall_co, begin, end)
  writeArchive("coTrainingVisao1EbALV2Dwsa_select.txt", "../", dataName, method,
               acc_co_v1, fscore_co_v1, preci_co_v1, recall_co_v1, begin, end)
  writeArchive("coTrainingVisao2EbALV2Dwsa_select.txt", "../", dataName, method,
