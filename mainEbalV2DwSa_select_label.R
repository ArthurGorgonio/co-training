...
meansFlexConC1S <- c()
method <- "Co-Training EbAL V2 DwS - A - Select and Label"
...
script_name <- "_CT_DwSA_Selecting_Labelling"
...
for (dataset in databases) {
  ...
  msg <- paste("DataSet[", bd, "]:\t", dataName, "\nParams:\n\t% Label: ",
                ...
                "%\n\tEbal concordance: ", ebal_concordance * 100, '%\n',
                sep="")
  cat(msg)
  ...
  for (fold in folds) {
    ...
    co_training <- coTrainingEbalV2DwcaSelectLabel(myModel, myFuncs, data1, data2)
  ...
  writeArchive("coTrainingMediaEbALV2Dwsa_select_label.txt", "../", dataName, method, acc_co,
               fscore_co, preci_co, recall_co, begin, end)
  writeArchive("coTrainingVisao1EbALV2Dwsa_select_label.txt", "../", dataName, method,
               acc_co_v1, fscore_co_v1, preci_co_v1, recall_co_v1, begin, end)
  writeArchive("coTrainingVisao2EbALV2Dwsa_select_label.txt", "../", dataName, method,
